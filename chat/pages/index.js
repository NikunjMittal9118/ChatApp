...
export default function index() {
  ...
  function Send() {
    const id = uuid();
    setSendIds([...sendIds, id]);
    socket.emit("message", { id, user: userName, msg: messageInput });
    setMessageInput("");
  }

  const isSent = (id) => {
    let ind = sendIds.indexOf(id);
    return ind === -1;
  };
  ...
  return (
    <div>
      ...
      <div>
        ...
        {messages.filter((msg) => isSent(msg.id)).map((message, index) => (
            <p key={index}>[{now.toLocaleTimeString()}] {message.userName}: {message.msg}</p>
          ))}
      </div>
      <div>
        <h2>Sent Messages:</h2>
        {messages.filter((msg) => !isSent(msg.id)).map((message, index) => (
            <p key={index}>[{now.toLocaleTimeString()}] {userName}: {message.msg}</p>
          ))}
      </div>
    </div>
  );
